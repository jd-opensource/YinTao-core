...
import {
  Dispatcher, DispatcherScope, existingDispatcher, lookupDispatcher, lookupNullableDispatcher,
} from './dispatcher'
...
import {
  RequestDispatcher, ResponseDispatcher, RouteDispatcher, WebSocketDispatcher,
...
export class PageDispatcher extends Dispatcher<Page, channels.PageChannel> implements channels.PageChannel {
  ...
  constructor(scope: DispatcherScope, page: Page) {
    ...
    if (page._video) { this._dispatchEvent('video', { artifact: existingDispatcher<ArtifactDispatcher>(page._video) }) }
