...
import {
  RouteDispatcher, RequestDispatcher, ResponseDispatcher, APIRequestContextDispatcher,
} from './networkDispatchers'
...
export class BrowserContextDispatcher extends Dispatcher<BrowserContext, channels.BrowserContextChannel> implements channels.BrowserContextChannel {
  ...
  constructor(scope: DispatcherScope, context: BrowserContext) {
    ...
    for (const page of context.pages()) { this._dispatchEvent('page', { page: new PageDispatcher(this._scope, page) }) }
    ...
    if (context._browser.options.name === 'chromium') {
      for (const page of (context as CRBrowserContext).backgroundPages()) { this._dispatchEvent('backgroundPage', { page: new PageDispatcher(this._scope, page) }) }
      ...
      for (const serviceWorker of (context as CRBrowserContext).serviceWorkers()) { this._dispatchEvent('serviceWorker', { worker: new WorkerDispatcher(this._scope, serviceWorker) }) }
