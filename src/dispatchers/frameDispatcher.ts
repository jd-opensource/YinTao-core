...
import {
  Dispatcher, DispatcherScope, lookupNullableDispatcher, existingDispatcher,
} from './dispatcher'
...
export class FrameDispatcher extends Dispatcher<Frame, channels.FrameChannel> implements channels.FrameChannel {
  ...
  private constructor(scope: DispatcherScope, frame: Frame) {
    ...
    frame.on(Frame.Events.Navigation, (event: NavigationEvent) => {
      ...
      if (event.newDocument) { (params as any).newDocument = { request: RequestDispatcher.fromNullable(this._scope, event.newDocument.request || null) } }
  ...
  async expect(params: channels.FrameExpectParams, metadata: CallMetadata): Promise<channels.FrameExpectResult> {
    ...
    if (result.received !== undefined) { result.received = serializeResult(result.received) }
    if (result.matches === params.isNot) { metadata.error = { error: { name: 'Expect', message: 'Expect failed' } } }
    return result
  }
}
