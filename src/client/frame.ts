...
import {
  assertMaxArguments, JSHandle, serializeArgument, parseResult,
} from './jsHandle'
...
import {
  LifecycleEvent, URLMatch, SelectOption, SelectOptionOptions, FilePayload, WaitForFunctionOptions, StrictOptions, kLifecycleEvents,
} from './types'
...
export class Frame extends ChannelOwner<channels.FrameChannel> implements api.Frame {
  ...
  async waitForURL(url: URLMatch, options: { waitUntil?: LifecycleEvent, timeout?: number } = {}): Promise<void> {
    if (urlMatches(this._page?.context()._options.baseURL, this.url(), url)) { return await this.waitForLoadState(options.waitUntil, options) }
  ...
  async dispatchEvent(selector: string, type: string, eventInit?: any, options: channels.FrameDispatchEventOptions = {}): Promise<void> {
    await this._channel.dispatchEvent({
      selector, type, eventInit: serializeArgument(eventInit), ...options,
    })
  }
  ...
  async $eval<R, Arg>(selector: string, pageFunction: structs.PageFunctionOn<Element, Arg, R>, arg?: Arg): Promise<R> {
    ...
    const result = await this._channel.evalOnSelector({
      selector, expression: String(pageFunction), isFunction: typeof pageFunction === 'function', arg: serializeArgument(arg),
    })
    return parseResult(result.value)
  }
  ...
  async getAttribute(selector: string, name: string, options: channels.FrameGetAttributeOptions = {}): Promise<string | null> {
    const { value } = await this._channel.getAttribute({ selector, name, ...options })
    return value === undefined ? null : value
  }
...
export function verifyLoadState(name: string, waitUntil: LifecycleEvent): LifecycleEvent {
  ...
  if (!kLifecycleEvents.has(waitUntil)) { throw new Error(`${name}: expected one of (load|domcontentloaded|networkidle|commit)`) }
  return waitUntil
}
