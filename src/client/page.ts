...
import {
  assert,
  isString, isRegExp, isObject, mkdirIfNeeded, headersObjectToArray,
} from '../utils/utils'
...
import {
  assertMaxArguments, serializeArgument, parseResult, JSHandle,
} from './jsHandle'
import {
  Request, Response, Route, RouteHandlerCallback, WebSocket, validateHeaders, RouteHandler,
} from './network'
...
import {
  Size, URLMatch, Headers, LifecycleEvent, WaitForEventOptions, SelectOption, SelectOptionOptions, FilePayload, WaitForFunctionOptions,
} from './types'
...
export class Page extends ChannelOwner<channels.PageChannel> implements api.Page {
  ...
  private _forceVideo(): Video {
    if (!this._video) { this._video = new Video(this, this._connection) }
    return this._video
  }
  ...
  private async _waitForEvent(event: string, optionsOrPredicate: WaitForEventOptions, logLine?: string): Promise<any> {
    return this._wrapApiCall(async () => {
      ...
      if (event !== Events.Page.Crash) { waiter.rejectOnEvent(this, Events.Page.Crash, new Error('Page crashed')) }
      if (event !== Events.Page.Close) { waiter.rejectOnEvent(this, Events.Page.Close, new Error('Page closed')) }
  ...
  override on(event: string | symbol, listener: Listener): this {
    if (event === Events.Page.FileChooser && !this.listenerCount(event)) { this._channel.setFileChooserInterceptedNoReply({ intercepted: true }) }
  ...
  override off(event: string | symbol, listener: Listener): this {
    ...
    if (event === Events.Page.FileChooser && !this.listenerCount(event)) { this._channel.setFileChooserInterceptedNoReply({ intercepted: false }) }
    return this
  }
...
export class BindingCall extends ChannelOwner<channels.BindingCallChannel> {
  ...
  async call(func: (source: structs.BindingSource, ...args: any[]) => any) {
    try {
      ...
      let result: any
      if (this._initializer.handle) { result = await func(source, JSHandle.from(this._initializer.handle)) } else { result = await func(source, ...this._initializer.args!.map(parseResult)) }
...
function trimUrl(param: any): string | undefined {
  if (isRegExp(param)) { return `/${trimEnd(param.source)}/${param.flags}` }
  if (isString(param)) { return `"${trimEnd(param)}"` }
}
