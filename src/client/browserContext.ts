...
import {
  URLMatch, Headers, WaitForEventOptions, BrowserContextOptions, StorageState, LaunchOptions,
} from './types'
...
export class BrowserContext extends ChannelOwner<channels.BrowserContextChannel> implements api.BrowserContext {
  ...
  async waitForEvent(event: string, optionsOrPredicate: WaitForEventOptions = {}): Promise<any> {
    return this._wrapApiCall(async () => {
      ...
      if (event !== Events.BrowserContext.Close) { waiter.rejectOnEvent(this, Events.BrowserContext.Close, new Error('Context closed')) }
  ...
  async newCDPSession(page: Page | Frame): Promise<api.CDPSession> {
    ...
    if (!(page instanceof Page) && !(page instanceof Frame)) { throw new Error('page: expected Page or Frame') }
