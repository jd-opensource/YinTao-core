...
export abstract class ChannelOwner<T extends channels.Channel = channels.Channel> extends EventEmitter {
  ...
  private _createChannel(base: Object): T {
    const channel = new Proxy(base, {
      get: (obj: any, prop) => {
        if (prop === 'debugScopeState') { return (params: any) => this._connection.sendMessageToServer(this, prop, params, null) }
        if (typeof prop === 'string') {
          const validator = scheme[paramsName(this._type, prop)]
          if (validator) {
            return (params: any) => this._wrapApiCall((apiZone) => {
  ...
  async _wrapApiCall<R>(func: (apiZone: ApiZone) => Promise<R>, isInternal = false): Promise<R> {
    ...
    try {
      ...
      const apiZone = {
        stackTrace, isInternal, reported: false, csi, callCookie,
      }
      const result = await zones.run<ApiZone, R>('apiZone', apiZone, async () => await func(apiZone))
...
function logApiCall(logger: Logger | undefined, message: string, isNested: boolean) {
  if (isNested) { return }
  if (logger && logger.isEnabled('api', 'info')) { logger.log('api', 'info', message, [], { color: 'cyan' }) }
...
const tChannel = (name: string): Validator => (arg: any, path: string) => {
  if (arg._object instanceof ChannelOwner && (name === '*' || arg._object._type === name)) return { guid: arg._object._guid }
