...
import {
  Headers, RemoteAddr, SecurityDetails, WaitForEventOptions,
} from './types'
...
export class Request extends ChannelOwner<channels.RequestChannel> implements api.Request {
  ...
  _actualHeaders(): Promise<RawHeaders> {
    if (!this._actualHeadersPromise) {
      this._actualHeadersPromise = this._wrapApiCall(async () => new RawHeaders((await this._channel.rawRequestHeaders()).headers))
    }
  ...
  async _internalResponse(): Promise<Response | null> {
    return this._wrapApiCall(async () => Response.fromNullable((await this._channel.response()).response), true)
  }
  ...
  async fulfill(options: { response?: api.APIResponse, status?: number, headers?: Headers, contentType?: string, body?: string | Buffer, path?: string } = {}) {
    ...
    if (options.response) {
      ...
      if (options.body === undefined && options.path === undefined && options.response instanceof APIResponse) {
        if (options.response._request._connection === this._connection) { fetchResponseUid = (options.response as APIResponse)._fetchUid() } else { body = await options.response.body() }
      }
    }
...
export class Response extends ChannelOwner<channels.ResponseChannel> implements api.Response {
  ...
  async _actualHeaders(): Promise<RawHeaders> {
    if (!this._actualHeadersPromise) {
      this._actualHeadersPromise = (async () => new RawHeaders((await this._channel.rawResponseHeaders()).headers))()
    }
    return this._actualHeadersPromise
  }
...
export class WebSocket extends ChannelOwner<channels.WebSocketChannel> implements api.WebSocket {
  ...
  constructor(parent: ChannelOwner, type: string, guid: string, initializer: channels.WebSocketInitializer) {
    ...
    this._channel.on('frameSent', (event: { opcode: number, data: string }) => {
      if (event.opcode === 1) { this.emit(Events.WebSocket.FrameSent, { payload: event.data }) } else if (event.opcode === 2) { this.emit(Events.WebSocket.FrameSent, { payload: Buffer.from(event.data, 'base64') }) }
    })
    this._channel.on('frameReceived', (event: { opcode: number, data: string }) => {
      if (event.opcode === 1) { this.emit(Events.WebSocket.FrameReceived, { payload: event.data }) } else if (event.opcode === 2) { this.emit(Events.WebSocket.FrameReceived, { payload: Buffer.from(event.data, 'base64') }) }
    })
  ...
  async waitForEvent(event: string, optionsOrPredicate: WaitForEventOptions = {}): Promise<any> {
    return this._wrapApiCall(async () => {
      ...
      if (event !== Events.WebSocket.Error) { waiter.rejectOnEvent(this, Events.WebSocket.Error, new Error('Socket error')) }
      if (event !== Events.WebSocket.Close) { waiter.rejectOnEvent(this, Events.WebSocket.Close, new Error('Socket closed')) }
...
export function validateHeaders(headers: Headers) {
  for (const key of Object.keys(headers)) {
    ...
    if (!Object.is(value, undefined) && !isString(value)) { throw new Error(`Expected value of header "${key}" to be String, but "${typeof value}" is found.`) }
  }
}
