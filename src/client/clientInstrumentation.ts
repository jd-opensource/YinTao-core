...
export function createInstrumentation(): ClientInstrumentation {
  ...
  return new Proxy({}, {
    get: (obj: any, prop: string) => {
      if (prop === 'addListener') { return (listener: ClientInstrumentationListener) => listeners.push(listener) }
      if (prop === 'removeListener') { return (listener: ClientInstrumentationListener) => listeners.splice(listeners.indexOf(listener), 1) }
