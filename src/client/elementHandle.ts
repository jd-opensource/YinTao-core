...
import {
  SelectOption, FilePayload, Rect, SelectOptionOptions,
} from './types'
...
export class ElementHandle<T extends Node = Node> extends JSHandle<T> implements api.ElementHandle {
  ...
  async getAttribute(name: string): Promise<string | null> {
    const { value } = await this._elementChannel.getAttribute({ name })
  ...
  async textContent(): Promise<string | null> {
    const { value } = await this._elementChannel.textContent()
    return value === undefined ? null : value
  }
  ...
  async boundingBox(): Promise<Rect | null> {
    const { value } = await this._elementChannel.boundingBox()
    return value === undefined ? null : value
  }
...
export function convertSelectOptionValues(values: string | api.ElementHandle | SelectOption | string[] | api.ElementHandle[] | SelectOption[] | null): { elements?: channels.ElementHandleChannel[], options?: SelectOption[] } {
  ...
  for (let i = 0; i < values.length; i++) { assert(values[i] !== null, `options[${i}]: expected object, got null`) }
  if (values[0] instanceof ElementHandle) { return { elements: (values as ElementHandle[]).map((v: ElementHandle) => v._elementChannel) } }
