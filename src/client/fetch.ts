...
import {
  assert, headersObjectToArray, isFilePayload, isString, mkdirIfNeeded, objectToArray,
} from '../utils/utils'
...
export class APIRequestContext extends ChannelOwner<channels.APIRequestContextChannel> implements api.APIRequestContext {
  ...
  async fetch(urlOrRequest: string | api.Request, options: FetchOptions = {}): Promise<APIResponse> {
    return this._wrapApiCall(async () => {
      ...
      if (postDataBuffer === undefined && jsonData === undefined && formData === undefined && multipartData === undefined) { postDataBuffer = request?.postDataBuffer() || undefined }
...
export class APIResponse implements api.APIResponse {
  ...
  async body(): Promise<Buffer> {
    try {
      ...
      if (e.message.includes(kBrowserOrContextClosedError)) { throw new Error('Response has been disposed') }
      throw e
    }
  }
