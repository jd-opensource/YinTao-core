...
export class Connection extends EventEmitter {
  ...
  dispatch(message: object) {
    ...
    const {
      id, guid, method, params, result, error,
    } = message as any
    if (id) {
      ...
      if (error && !result) { callback.reject(parseError(error)) } else { callback.resolve(this._replaceGuidsWithChannels(result)) }
      return
    }
  ...
  private _createRemoteObject(parentGuid: string, type: string, guid: string, initializer: any): any {
    ...
    switch (type) {
      case 'Android':
        ...
        break
      case 'AndroidSocket':
        result = null as any // new AndroidSocket(parent, type, guid, initializer)
        break
      case 'AndroidDevice':
        result = null as any // new AndroidDevice(parent, type, guid, initializer)
        break
