...
import {
  LaunchOptions, LaunchServerOptions, ConnectOptions, LaunchPersistentContextOptions, BrowserContextOptions,
} from './types'
...
export class BrowserType extends ChannelOwner<channels.BrowserTypeChannel> implements api.BrowserType {
  ...
  async _connect(wsEndpoint: string, params: Partial<ConnectOptions> = {}): Promise<Browser> {
    ...
    return await this._wrapApiCall(async () => {
      ...
      let browser: Browser
      const connectParams: channels.BrowserTypeConnectParams = {
        wsEndpoint, headers: params.headers, slowMo: params.slowMo, timeout: params.timeout,
      }
      if ((params as any).__testHookRedirectPortForwarding) connectParams.socksProxyRedirectPortForTest = (params as any).__testHookRedirectPortForwarding
      ...
      const result = await raceAgainstTimeout(async () => {
        // For tests.
        if ((params as any).__testHookBeforeCreateBrowser) await (params as any).__testHookBeforeCreateBrowser()
  ...
  async _connectOverCDP(endpointURL: string, params: api.ConnectOverCDPOptions = {}): Promise<Browser> {
    ...
    if (result.defaultContext) { browser._contexts.add(BrowserContext.from(result.defaultContext)) }
