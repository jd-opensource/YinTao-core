...
import type {
  ElementStateWithoutStable, FrameExpectParams, InjectedScriptPoll, InjectedScriptProgress,
...
export class FrameManager {
  ...
  private _removeFramesRecursively(frame: Frame) {
    ...
    if (!this._page.isClosed()) { this._page.emit(Page.Events.FrameDetached, frame) }
  }
  ...
  onWebSocketRequest(requestId: string) {
    ...
    if (ws && ws.markAsNotified()) { this._page.emit(Page.Events.WebSocket, ws) }
  }
...
export class Frame extends SdkObject {
  ...
  private _recalculateLifecycle() {
    ...
    for (const event of events) {
      ...
      if (!this._subtreeLifecycleEvents.has(event)) {
        ...
        if (this === mainFrame && this._url !== 'about:blank') { debugLogger.log('api', `  "${event}" event fired`) }
        if (this === mainFrame && event === 'load') { this._page.emit(Page.Events.Load) }
        if (this === mainFrame && event === 'domcontentloaded') { this._page.emit(Page.Events.DOMContentLoaded) }
      }
    }
    ...
    return this.raceNavigationAction(async () => {
      ...
      if (navigateResult.newDocumentId) {
        ...
        event = await helper.waitForEvent(progress, this, Frame.Events.Navigation, (event: NavigationEvent) =>
          ...
          event.newDocument && (event.newDocument.documentId === navigateResult.newDocumentId || !event.error)).promise
      ...
      if (!this._subtreeLifecycleEvents.has(waitUntil)) { await helper.waitForEvent(progress, this, Frame.Events.AddLifecycle, (e: types.LifecycleEvent) => e === waitUntil).promise }
  ...
  async waitForSelector(metadata: CallMetadata, selector: string, options: types.WaitForElementOptions & { omitReturnValue?: boolean }, scope?: dom.ElementHandle): Promise<dom.ElementHandle<Element> | null> {
    ...
    return controller.run(async (progress) => {
      ...
      return this.retryWithProgress(progress, selector, options, async (selectorInFrame, continuePolling) => {
        ...
        if ((options as any).__testHookBeforeAdoptNode) { await (options as any).__testHookBeforeAdoptNode() }
  ...
  async content(): Promise<string> {
    try {
      ...
      if (js.isJavaScriptErrorInEvaluate(e) || isSessionClosedError(e)) { throw e }
  ...
  async addScriptTag(params: {
    ...
    return this._raceWithCSPError(async () => {
      if (url !== null) { return (await context.evaluateHandle(addScriptUrl, { url, type })).asElement()! }
  ...
  async retryWithProgress<R>(
    ...
    while (progress.isRunning()) {
      ...
      try {
        ...
        if (scope && scope._context.frame === selectorInFrame?.frame) { throw e }
  ...
  async click(metadata: CallMetadata, selector: string, options: types.MouseClickOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._click(progress, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async dblclick(metadata: CallMetadata, selector: string, options: types.MouseMultiClickOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions = {}) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._dblclick(progress, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async dragAndDrop(metadata: CallMetadata, source: string, target: string, options: types.DragActionOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions = {}) {
    ...
    await controller.run(async (progress) => {
      dom.assertDone(await this._retryWithProgressIfNotConnected(progress, source, options.strict, async (handle) => handle._retryPointerAction(progress, 'move and down', false, async (point) => {
      ...
      dom.assertDone(await this._retryWithProgressIfNotConnected(progress, target, options.strict, async (handle) => handle._retryPointerAction(progress, 'move and up', false, async (point) => {
        await this._page.mouse.move(point.x, point.y)
  ...
  async tap(metadata: CallMetadata, selector: string, options: types.PointerActionWaitOptions & types.NavigatingActionWaitOptions) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._tap(progress, options))), this._page._timeoutSettings.timeout(options))
  }

  async fill(metadata: CallMetadata, selector: string, value: string, options: types.NavigatingActionWaitOptions & { force?: boolean }) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._fill(progress, value, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async highlight(selector: string) {
    ...
    const injectedScript = await context.injectedScript()
    return await injectedScript.evaluate((injected, { parsed }) => injected.highlight(parsed), { parsed: pair.info.parsed })
  }

  async hideHighlight() {
    const context = await this._utilityContext()
    const injectedScript = await context.injectedScript()
    return await injectedScript.evaluate((injected) => injected.hideHighlight())
  }
  ...
  async hover(metadata: CallMetadata, selector: string, options: types.PointerActionOptions & types.PointerActionWaitOptions = {}) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._hover(progress, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async selectOption(metadata: CallMetadata, selector: string, elements: dom.ElementHandle[], values: types.SelectOption[], options: types.NavigatingActionWaitOptions & types.ForceOptions = {}): Promise<string[]> {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._selectOption(progress, elements, values, options)), this._page._timeoutSettings.timeout(options))
  }
  ...
  async setInputFiles(metadata: CallMetadata, selector: string, files: channels.ElementHandleSetInputFilesParams['files'], options: types.NavigatingActionWaitOptions = {}): Promise<void> {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._setInputFiles(progress, files, options))), this._page._timeoutSettings.timeout(options))
  }

  async type(metadata: CallMetadata, selector: string, text: string, options: { delay?: number } & types.NavigatingActionWaitOptions = {}) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._type(progress, text, options))), this._page._timeoutSettings.timeout(options))
  }

  async press(metadata: CallMetadata, selector: string, key: string, options: { delay?: number } & types.NavigatingActionWaitOptions = {}) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._press(progress, key, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async check(metadata: CallMetadata, selector: string, options: types.PointerActionWaitOptions & types.NavigatingActionWaitOptions = {}) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._setChecked(progress, true, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async uncheck(metadata: CallMetadata, selector: string, options: types.PointerActionWaitOptions & types.NavigatingActionWaitOptions = {}) {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => dom.assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, (handle) => handle._setChecked(progress, false, options))), this._page._timeoutSettings.timeout(options))
  }
  ...
  async expect(metadata: CallMetadata, selector: string, options: FrameExpectParams): Promise<{ matches: boolean, received?: any, log?: string[] }> {
    ...
    return controller.run(async (outerProgress) => {
      ...
      return await this._scheduleRerunnableTaskWithProgress(outerProgress, selector, (progress, element, options, elements) => {
        ...
        strict: true, querySelectorAll: isArray, mainWorld, omitAttached, logScale: true, ...options,
      })
    }, timeout).catch((e) => {
      // Q: Why not throw upon isSessionClosedError(e) as in other places?
      // A: We want user to receive a friendly message containing the last intermediate result.
      if (js.isJavaScriptErrorInEvaluate(e) || isInvalidSelectorError(e)) { throw e }
  ...
  async _waitForFunctionExpression<R>(metadata: CallMetadata, expression: string, isFunction: boolean | undefined, arg: any, options: types.WaitForFunctionOptions, world: types.World = 'main'): Promise<js.SmartHandle<R>> {
    const controller = new ProgressController(metadata, this)
    if (typeof options.pollingInterval === 'number') { assert(options.pollingInterval > 0, `Cannot poll with non-positive interval: ${options.pollingInterval}`) }
    expression = js.normalizeEvaluationExpression(expression, isFunction)
    const task: dom.SchedulableTask<R> = (injectedScript) => injectedScript.evaluateHandle((injectedScript, {
      expression, isFunction, polling, arg,
    }) => {
  ...
  private async _scheduleRerunnableTask<T, R>(metadata: CallMetadata, selector: string, body: DomTaskBody<T, R, Element>, taskData: T, options: types.TimeoutOptions & types.StrictOptions & { mainWorld?: boolean } = {}): Promise<R> {
    const controller = new ProgressController(metadata, this)
    return controller.run(async (progress) => await this._scheduleRerunnableTaskWithProgress(progress, selector, body as DomTaskBody<T, R, Element | undefined>, taskData, options), this._page._timeoutSettings.timeout(options))
  }
  ...
  private async _scheduleRerunnableTaskWithProgress<T, R>(
    ...
    taskData: T,
    options: types.TimeoutOptions & types.StrictOptions & { mainWorld?: boolean, querySelectorAll?: boolean, logScale?: boolean, omitAttached?: boolean } = {},
  ): Promise<R> {
  ...
  private async _scheduleRerunnableTaskInFrame<T, R>(
    ...
    {
      const rerunnableTask = new RerunnableTask<R>(data, progress, (injectedScript) => injectedScript.evaluateHandle((injected, {
        info, taskData, callbackText, querySelectorAll, logScale, omitAttached, snapshotName,
      }) => {
  ...
  async extendInjectedScript(source: string, arg?: any): Promise<js.JSHandle> {
    ...
    return injectedScriptHandle.evaluateHandle((injectedScript, { source, arg }) => injectedScript.extend(source, arg), { source, arg })
  }
  ...
  private async _resolveFrameForSelector(progress: Progress, selector: string, options: types.StrictOptions & types.TimeoutOptions, scope?: dom.ElementHandle): Promise<SelectorInFrame | null> {
    ...
    for (let i = 0; i < frameChunks.length - 1 && progress.isRunning(); ++i) {
      ...
      if (!isIframe) { throw new Error(`Selector "${stringifySelector(info.parsed)}" resolved to ${element.preview()}, <iframe> was expected`) }
...
class RerunnableTask<T> {
  ...
  constructor(data: ContextData, progress: Progress, task: dom.SchedulableTask<T>, returnByValue: boolean) {
    ...
    if (returnByValue) { this.promise = new ManualPromise<T>() } else { this.handlePromise = new ManualPromise<js.SmartHandle<T>>() }
  ...
  private _resolve(value: T | js.SmartHandle<T>) {
    if (this.promise) { this.promise.resolve(value as T) }
    if (this.handlePromise) { this.handlePromise.resolve(value as js.SmartHandle<T>) }
  }
