...
import {
  pipeline, Readable, Transform, TransformCallback,
} from 'stream'
import url from 'url'
...
import {
  assert, createGuid, getUserAgent, monotonicTime,
} from '../utils/utils'
...
export abstract class APIRequestContext extends SdkObject {
  ...
  async fetch(params: channels.APIRequestContextFetchParams, metadata: CallMetadata): Promise<Omit<types.APIResponse, 'body'> & { fetchUid: string }> {
    ...
    const fetchResponse = await controller.run((progress) => this._sendRequest(progress, requestUrl, options, postData))
  ...
  private _parseSetCookieHeader(responseUrl: string, setCookie: string[] | undefined): types.NetworkCookie[] {
    ...
    for (const header of setCookie) {
      ...
      if (!cookie.domain) { cookie.domain = url.hostname } else { assert(cookie.domain.startsWith('.')) }
  ...
  private async _sendRequest(progress: Progress, url: URL, options: https.RequestOptions & { maxRedirects: number, deadline: number }, postData?: Buffer): Promise<types.APIResponse> {
    ...
    return new Promise<types.APIResponse>((fulfill, reject) => {
      const requestConstructor: ((url: URL, options: http.RequestOptions, callback?: (res: http.IncomingMessage) => void) => http.ClientRequest) = (url.protocol === 'https:' ? https : http).request
...
function toHeadersArray(rawHeaders: string[]): types.HeadersArray {
  ...
  for (let i = 0; i < rawHeaders.length; i += 2) { result.push({ name: rawHeaders[i], value: rawHeaders[i + 1] }) }
  return result
}
