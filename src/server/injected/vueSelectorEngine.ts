...
const classify = (str: string) => str && str.replace(classifyRE, toUpper)
...
function buildComponentsTreeVue3(instance: VueVNode): ComponentNode {
  ...
  function getInstanceName(instance: VueVNode): string {
    ...
    for (const key in instance.parent?.type?.components) { if (instance.parent?.type.components[key] === instance.type) return saveComponentName(instance, key) }
    for (const key in instance.appContext?.components) { if (instance.appContext.components[key] === instance.type) return saveComponentName(instance, key) }
...
function buildComponentsTreeVue2(instance: VueVNode): ComponentNode {
  ...
  function getInternalInstanceChildren(instance: VueVNode): VueVNode[] {
    ...
    if (Array.isArray(instance.subTree.children)) { return instance.subTree.children.filter((vnode: any) => !!vnode.component).map((vnode: any) => vnode.component) }
    return []
  }
...
function findVueRoots(root: Document | ShadowRoot, roots: VueRoot[] = []): VueRoot[] {
  ...
  do {
    ...
    if ((node as any).__vue_app__ && (node as any)._vnode && (node as any)._vnode.component) { roots.push({ root: (node as any)._vnode.component, version: 3 }) }
