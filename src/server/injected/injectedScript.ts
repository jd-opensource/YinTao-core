...
import {
  allEngineNames, ParsedSelector, ParsedSelectorPart, parseSelector, stringifySelector,
} from '../common/selectorParser'
import {
  SelectorEvaluatorImpl, isVisible, parentElementOrShadowHost, elementMatchesText, TextMatcher, createRegexTextMatcher, createStrictTextMatcher, createLaxTextMatcher,
...
export class InjectedScript {
  ...
  parseSelector(selector: string): ParsedSelector {
    ...
    for (const name of allEngineNames(result)) {
      if (!this._engines.has(name)) { throw this.createStacklessError(`Unknown engine "${name}" while parsing selector ${selector}`) }
    }
    return result
  }
  ...
  querySelector(selector: ParsedSelector, root: Node, strict: boolean): Element | undefined {
    ...
    try {
      ...
      if (strict && result.length > 1) { throw this.strictModeViolationError(selector, result.map((r) => r.element)) }
  ...
  private _querySelectorRecursively(roots: ElementMatch[], selector: ParsedSelector, index: number, queryCache: Map<Element, Element[][]>): ElementMatch[] {
    ...
    if (part.name === 'nth') {
      ...
      if (part.body === '0') {
        ...
        if (typeof selector.capture === 'number') { throw this.createStacklessError('Can\'t query n-th element in a request with the capture.') }
        const nth = +part.body
    ...
    for (const root of roots) {
      ...
      for (const element of all) {
        if (!('nodeName' in element)) { throw this.createStacklessError(`Expected a Node but got ${Object.prototype.toString.call(element)}`) }
  ...
  private _createAttributeEngine(attribute: string, shadow: boolean): SelectorEngine {
    ...
    return {
      queryAll: (root: SelectorRoot, selector: string): Element[] => this._evaluator.query({ scope: root as Document | Element, pierceShadow: shadow }, toCSS(selector)),
    }
  }
  ...
  private _createTextEngine(shadow: boolean): SelectorEngine {
    ...
    return {
      queryAll: (root: SelectorRoot, selector: string): Element[] => queryList(root, selector),
    }
  }
    ...
    return this._runAbortableTask((progress) => {
      ...
      const next = () => {
        ...
        try {
          ...
          if (success !== progress.continuePolling) { fulfill(success as T) } else { scheduleNext(next) }
        } catch (e) {
  ...
  private _runAbortableTask<T>(task: (progess: InjectedScriptProgress) => Promise<T>): InjectedScriptPoll<T> {
    ...
    let lastMessage = ''
    let lastIntermediateResult: any
  ...
  getElementBorderWidth(node: Node): { left: number; top: number; } {
    if (node.nodeType !== Node.ELEMENT_NODE || !node.ownerDocument || !node.ownerDocument.defaultView) { return { left: 0, top: 0 } }
  ...
  waitForElementStatesAndPerformAction<T>(
    ...
    return this.pollRaf((progress) => {
      ...
      for (const state of states) {
        ...
        const rect = {
          x: clientRect.top, y: clientRect.left, width: clientRect.width, height: clientRect.height,
        }
  ...
  elementState(node: Node, state: ElementStateWithoutStable): boolean | 'error:notconnected' {
    ...
    if (state === 'checked' || state === 'unchecked') {
      ...
      if (element.nodeName !== 'INPUT') { throw this.createStacklessError('Not a checkbox or radio button') }
  ...
  selectOptions(
    ...
    if (!element) { return 'error:notconnected' }
    if (element.nodeName.toLowerCase() !== 'select') { throw this.createStacklessError('Element is not a <select> element') }
  ...
  private _expectHitTargetParent(hitElement: Element | undefined, targetElement: Element) {
    ...
    if (rootHitTargetDescription) { return { hitTargetDescription: `${hitTargetDescription} from ${rootHitTargetDescription} subtree` } }
  ...
  dispatchEvent(node: Node, type: string, eventInit: Object) {
    let event
    eventInit = {
      bubbles: true, cancelable: true, composed: true, ...eventInit,
    }
  ...
  maskSelectors(selectors: ParsedSelector[]) {
    ...
    const elements:any[] = []
    for (const selector of selectors) { elements.push(this.querySelectorAll(selector, document.documentElement)) }
  ...
  private _setupHitTargetInterceptors() {
    ...
    const addHitTargetInterceptorListeners = () => {
      for (const event of kAllHitTargetInterceptorEvents) { window.addEventListener(event as any, listener, { capture: true, passive: false }) }
    }
  ...
  expectSingleElement(progress: InjectedScriptProgress, element: Element, options: FrameExpectParams): { matches: boolean, received?: any } {
    ...
    // eslint-disable-next-line no-lone-blocks
    {
      ...
      if (expression === 'to.be.checked') {
        ...
        if (element.nodeName === 'INPUT' || element.nodeName === 'TEXTAREA') { elementState = !(element as HTMLInputElement).value } else { elementState = !element.textContent?.trim() }
      ...
      if (elementState !== undefined) {
        if (elementState === 'error:notcheckbox') { throw injected.createStacklessError('Element is not a checkbox') }
        if (elementState === 'error:notconnected') { throw injected.createStacklessError('Element is not connected') }
    ...
    // eslint-disable-next-line no-lone-blocks
    {
      ...
      if (expression === 'to.have.attribute') {
        ...
        if (element.nodeName !== 'INPUT' && element.nodeName !== 'TEXTAREA' && element.nodeName !== 'SELECT') { throw this.createStacklessError('Not an input element') }
  ...
  expectArray(elements: Element[], options: FrameExpectParams): { matches: boolean, received?: any } {
    ...
    // List of values.
    let received: string[] | undefined
    if (expression === 'to.have.text.array' || expression === 'to.contain.text.array') { received = elements.map((e) => (options.useInnerText ? (e as HTMLElement).innerText : e.textContent || '')) } else if (expression === 'to.have.class.array') { received = elements.map((e) => e.className) }
