...
function generateSelectorFor(injectedScript: InjectedScript, targetElement: Element, strict: boolean): SelectorToken[] | null {
  if (targetElement.ownerDocument.documentElement === targetElement) { return [{ engine: 'css', selector: 'html', score: 1 }] }
...
function buildCandidates(injectedScript: InjectedScript, element: Element): SelectorToken[] {
  ...
  for (const attribute of ['data-testid', 'data-test-id', 'data-test']) {
    if (element.hasAttribute(attribute)) { candidates.push({ engine: 'css', selector: `[${attribute}=${quoteAttributeValue(element.getAttribute(attribute)!)}]`, score: 1 }) }
  }

  if (element.nodeName === 'INPUT') {
    const input = element as HTMLInputElement
    if (input.placeholder) { candidates.push({ engine: 'css', selector: `[placeholder=${quoteAttributeValue(input.placeholder)}]`, score: 10 }) }
  }
  if (element.hasAttribute('aria-label')) { candidates.push({ engine: 'css', selector: `[aria-label=${quoteAttributeValue(element.getAttribute('aria-label')!)}]`, score: 10 }) }
  if (element.getAttribute('alt') && ['APPLET', 'AREA', 'IMG', 'INPUT'].includes(element.nodeName)) { candidates.push({ engine: 'css', selector: `${cssEscape(element.nodeName.toLowerCase())}[alt=${quoteAttributeValue(element.getAttribute('alt')!)}]`, score: 10 }) }

  if (element.hasAttribute('role')) { candidates.push({ engine: 'css', selector: `${cssEscape(element.nodeName.toLowerCase())}[role=${quoteAttributeValue(element.getAttribute('role')!)}]`, score: 50 }) }

  if (element.getAttribute('name') && ['BUTTON', 'FORM', 'FIELDSET', 'IFRAME', 'INPUT', 'KEYGEN', 'OBJECT', 'OUTPUT', 'SELECT', 'TEXTAREA', 'MAP', 'META', 'PARAM'].includes(element.nodeName)) { candidates.push({ engine: 'css', selector: `${cssEscape(element.nodeName.toLowerCase())}[name=${quoteAttributeValue(element.getAttribute('name')!)}]`, score: 50 }) }
  if (['INPUT', 'TEXTAREA'].includes(element.nodeName) && element.getAttribute('type') !== 'hidden') {
    if (element.getAttribute('type')) { candidates.push({ engine: 'css', selector: `${cssEscape(element.nodeName.toLowerCase())}[type=${quoteAttributeValue(element.getAttribute('type')!)}]`, score: 50 }) }
  }
  if (['INPUT', 'TEXTAREA', 'SELECT'].includes(element.nodeName)) { candidates.push({ engine: 'css', selector: cssEscape(element.nodeName.toLowerCase()), score: 50 }) }

  const idAttr = element.getAttribute('id')
  if (idAttr && !isGuidLike(idAttr)) { candidates.push({ engine: 'css', selector: makeSelectorForId(idAttr), score: 100 }) }
...
function buildTextCandidates(injectedScript: InjectedScript, element: Element, allowHasText: boolean): SelectorToken[] {
  ...
  if (allowHasText && escaped === text) {
    ...
    if (element.hasAttribute('role')) { prefix += `[role=${quoteAttributeValue(element.getAttribute('role')!)}]` }
...
function parentElementOrShadowHost(element: Element): Element | null {
  ...
  if (element.parentNode.nodeType === Node.DOCUMENT_FRAGMENT_NODE && (element.parentNode as ShadowRoot).host) { return (element.parentNode as ShadowRoot).host }
  return null
}
...
function cssEscapeOne(s: string, i: number): string {
  ...
  if ((c >= 0x0001 && c <= 0x001f)
      || (c >= 0x0030 && c <= 0x0039 && (i === 0 || (i === 1 && s.charCodeAt(0) === 0x002d)))) { return `\\${c.toString(16)} ` }
  if (i === 0 && c === 0x002d && s.length === 1) { return `\\${s.charAt(i)}` }
  if (c >= 0x0080 || c === 0x002d || c === 0x005f || (c >= 0x0030 && c <= 0x0039)
      || (c >= 0x0041 && c <= 0x005a) || (c >= 0x0061 && c <= 0x007a)) { return s.charAt(i) }
  return `\\${s.charAt(i)}`
}
