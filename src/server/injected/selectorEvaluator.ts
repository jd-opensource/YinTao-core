...
export class SelectorEvaluatorImpl implements SelectorEvaluator {
  ...
  private _querySimple(context: QueryContext, simple: CSSSimpleSelector): Element[] {
    ...
    return this._cached<Element[]>(this._cacheQuerySimple, simple, [context.scope, context.pierceShadow], () => {
      ...
      if (css !== undefined) {
        ...
        if (hasScopeClause && context.scope.nodeType === 1 /* Node.ELEMENT_NODE */) { elements.unshift(context.scope as Element) }
      } else {
  ...
  private _callMatches(engine: SelectorEngine, element: Element, args: CSSFunctionArgument[], context: QueryContext): boolean {
    return this._cached<boolean>(this._cacheCallMatches, element, [engine, context.scope, context.pierceShadow, ...args], () => engine.matches!(element, args, context, this))
  }

  private _callQuery(engine: SelectorEngine, args: CSSFunctionArgument[], context: QueryContext): Element[] {
    return this._cached<Element[]>(this._cacheCallQuery, engine, [context.scope, context.pierceShadow, ...args], () => engine.query!(context, args, this))
  }
...
const scopeEngine: SelectorEngine = {
  matches(element: Element, args: (string | number | Selector)[], context: QueryContext): boolean {
    if (args.length !== 0) { throw new Error('"scope" engine expects no arguments') }
    if (context.scope.nodeType === 9 /* Node.DOCUMENT_NODE */) { return element === (context.scope as Document).documentElement }
    return element === context.scope
  },

  query(context: QueryContext, args: (string | number | Selector)[]): Element[] {
    if (args.length !== 0) { throw new Error('"scope" engine expects no arguments') }
...
const visibleEngine: SelectorEngine = {
  matches(element: Element, args: (string | number | Selector)[]): boolean {
...
export function createRegexTextMatcher(source: string, flags?: string): TextMatcher {
  ...
  return (elementText: ElementText) => re.test(elementText.full)
}
      ...
      if ((root instanceof HTMLInputElement) && (root.type === 'submit' || root.type === 'button')) {
        ...
        for (let child = root.firstChild; child; child = child.nextSibling) {
          if (child.nodeType === Node.TEXT_NODE) {
            ...
            currentImmediate = ''
            if (child.nodeType === Node.ELEMENT_NODE) { value.full += elementText(evaluator, child as Element).full }
          }
        }
        if (currentImmediate) { value.immediate.push(currentImmediate) }
        if ((root as Element).shadowRoot) { value.full += elementText(evaluator, (root as Element).shadowRoot!).full }
      }
    }
...
export function elementMatchesText(evaluator: SelectorEvaluatorImpl, element: Element, matcher: TextMatcher): 'none' | 'self' | 'selfAndChildren' {
  ...
  for (let child = element.firstChild; child; child = child.nextSibling) {
    if (child.nodeType === Node.ELEMENT_NODE && matcher(elementText(evaluator, child as Element))) { return 'selfAndChildren' }
  }
...
export function isVisible(element: Element): boolean {
  ...
  if (style.display === 'contents') {
    ...
    for (let child = element.firstChild; child; child = child.nextSibling) {
      if (child.nodeType === 1 /* Node.ELEMENT_NODE */ && isVisible(child as Element)) { return true }
