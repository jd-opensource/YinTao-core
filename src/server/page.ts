...
export class Page extends SdkObject {
  ...
  async _doSlowMo() {
    const { slowMo } = this._browserContext._browser.options
    if (!slowMo) { return }
  ...
  async expectScreenshot(metadata: CallMetadata, options: ExpectScreenshotOptions = {}): Promise<{ actual?: Buffer, previous?: Buffer, diff?: Buffer, errorMessage?: string, log?: string[] }> {
    ...
    const rafrafScreenshot = locator ? async (progress: Progress, timeout: number) => await locator.frame.rafrafTimeoutScreenshotElementWithProgress(progress, locator.selector, timeout, options.screenshotOptions || {}) : async (progress: Progress, timeout: number) => {
    ...
    if (isGeneratingNewScreenshot && options.isNot) { return { errorMessage: '"not" matcher requires expected result' } }
    ...
    return controller.run(async (progress) => {
      ...
      while (true) {
        ...
        if (comparatorResult) {
          intermediateResult = {
            errorMessage: comparatorResult.errorMessage, diff: comparatorResult.diff, actual, previous,
          }
        }
      }
    ...
    try {
      ...
      if (isError(error)) {
        context.evaluate(deliverError, {
          name, seq, message: error.message, stack: error.stack,
        }).catch((e) => debugLogger.log('error', e))
      } else { context.evaluate(deliverErrorValue, { name, seq, error }).catch((e) => debugLogger.log('error', e)) }
    }
