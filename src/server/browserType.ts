...
import {
  BrowserOptions, Browser, BrowserProcess, PlaywrightOptions,
} from './browser'
...
export abstract class BrowserType extends SdkObject {
  ...
  async launchPersistentContext(metadata: CallMetadata, userDataDir: string, options: types.LaunchPersistentOptions): Promise<BrowserContext> {
    ...
    controller.setLogName('browser')
    const browser = await controller.run((progress) => this._innerLaunchWithRetries(progress, options, persistent, helper.debugProtocolLogger(), userDataDir).catch((e) => { throw this._rewriteStartupError(e) }), TimeoutSettings.timeout(options))
  ...
  async _innerLaunch(progress: Progress, options: types.LaunchOptions, persistent: types.BrowserContextOptions | undefined, protocolLogger: types.ProtocolLogger, userDataDir?: string): Promise<Browser> {
    ...
    if ((options as any).__testHookBeforeCreateBrowser) { await (options as any).__testHookBeforeCreateBrowser() }
  ...
  private _validateLaunchOptions<Options extends types.LaunchOptions>(options: Options): Options {
    ...
    if (downloadsPath && !path.isAbsolute(downloadsPath)) { downloadsPath = path.join(process.cwd(), downloadsPath) }
    if (this._playwrightOptions.socksProxyPort) { proxy = { server: `socks5://127.0.0.1:${this._playwrightOptions.socksProxyPort}` } }
    return {
      ...options, devtools, headless, downloadsPath, proxy,
    }
  }
