...
export abstract class BrowserContext extends SdkObject {
  ...
  constructor(browser: Browser, options: types.BrowserContextOptions, browserContextId: string | undefined) {
    ...
    if (this._options.recordHar) { this._harRecorder = new HarRecorder(this, { ...this._options.recordHar, path: path.join(this._browser.options.artifactsDir, `${createGuid()}.har`) }) }
  ...
  async _initialize() {
    ...
    if (debugMode() === 'inspector') { await RecorderSupplement.show(this, { pauseOnNextStatement: true }) }
  ...
  async _ensureVideosPath() {
    if (this._options.recordVideo) { await mkdirIfNeeded(path.join(this._options.recordVideo.dir, 'dummy')) }
  }
...
export function validateBrowserContextOptions(options: types.BrowserContextOptions, browserOptions: BrowserOptions) {
  ...
  if (!options.viewport && !options.noDefaultViewport) { options.viewport = { width: 1280, height: 720 } }
