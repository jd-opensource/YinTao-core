...
import type {
  InjectedScript, InjectedScriptPoll, LogEntry, HitTargetInterceptionResult,
...
export class FrameExecutionContext extends js.ExecutionContext {
  ...
  async evaluateExpressionAndWaitForSignals(expression: string, isFunction: boolean | undefined, arg?: any): Promise<any> {
    return await this.frame._page._frameManager.waitForSignalsCreatedBy(null, false /* noWaitFor */, async () => this.evaluateExpression(expression, isFunction, arg))
  }

  async evaluateExpressionHandleAndWaitForSignals(expression: string, isFunction: boolean | undefined, arg: any): Promise<any> {
    return await this.frame._page._frameManager.waitForSignalsCreatedBy(null, false /* noWaitFor */, async () => js.evaluateExpression(this, false /* returnByValue */, expression, isFunction, arg))
  }
  ...
  injectedScript(): Promise<js.JSHandle<InjectedScript>> {
    if (!this._injectedScriptPromise) {
      const custom: string[] = []
      for (const [name, { source }] of this.frame._page.selectors._engines) { custom.push(`{ name: '${name}', engine: (${source}) }`) }
      const source = `
        (() => {
...
export class ElementHandle<T extends Node = Node> extends js.JSHandle<T> {
  ...
  async textContent(): Promise<string | null> {
    return throwRetargetableDOMError(await this.evaluateInUtility(([injected, node]) => ({ value: node.textContent }), undefined)).value
  }

  async innerText(): Promise<string> {
    return throwRetargetableDOMError(await this.evaluateInUtility(([injected, node]) => {
      if (node.nodeType !== Node.ELEMENT_NODE) { throw injected.createStacklessError('Node is not an element') }
      if ((node as unknown as Element).namespaceURI !== 'http://www.w3.org/1999/xhtml') { throw injected.createStacklessError('Node is not an HTMLElement') }
  ...
  async dispatchEvent(type: string, eventInit: Object = {}) {
    ...
    await this._page._frameManager.waitForSignalsCreatedBy(null, false /* noWaitFor */, async () => main.evaluate(([injected, node, { type, eventInit }]) => injected.dispatchEvent(node, type, eventInit), [await main.injectedScript(), this, { type, eventInit }] as const))
  ...
  private async _clickablePoint(): Promise<types.Point | 'error:notvisible' | 'error:notinviewport'> {
    const intersectQuadWithViewport = (quad: types.Quad): types.Quad => quad.map((point) => ({
  ...
  async _retryPointerAction(
    ...
    options: types.PointerActionOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions,
  ...
  async _performPointerAction(progress: Progress, actionName: ActionName, waitForEnabled: boolean, action: (point: types.Point) => Promise<void>, forceScrollOptions: ScrollIntoViewOptions | undefined, options: types.PointerActionOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions): Promise<'error:notvisible' | 'error:notconnected' | 'error:notinviewport' | { hitTargetDescription: string } | 'done'> {
    ...
    if ((options as any).__testHookBeforeStable) await (options as any).__testHookBeforeStable()
    const result = await this._waitForDisplayedAtStablePosition(progress, force, waitForEnabled)
    if (result !== 'done') return result
    if ((options as any).__testHookAfterStable) await (options as any).__testHookAfterStable()

    progress.log('  scrolling into view if needed')
    progress.throwIfAborted() // Avoid action that has side-effects.
    if (forceScrollOptions) {
      const scrolled = await this.evaluateInUtility(([injected, node, options]) => {
        if (node.nodeType === 1 /* Node.ELEMENT_NODE */) (node as Node as Element).scrollIntoView(options)
      }, forceScrollOptions)
    ...
    if (process.env.PLAYWRIGHT_NO_LAYOUT_SHIFT_CHECK) return this._finishPointerAction(progress, actionName, point, options, action)
  ...
  private async _finishPointerAction(progress: Progress, actionName: ActionName, point: types.Point, options: types.PointerActionOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions, action: (point: types.Point) => Promise<void>): Promise<'error:notconnected' | { hitTargetDescription: string } | 'done'> {
    if (!options.force) {
      if ((options as any).__testHookBeforeHitTarget) { await (options as any).__testHookBeforeHitTarget() }
    ...
    await this._page._frameManager.waitForSignalsCreatedBy(progress, options.noWaitAfter, async () => {
      if ((options as any).__testHookBeforePointerAction) { await (options as any).__testHookBeforePointerAction() }
      progress.throwIfAborted() // Avoid action that has side-effects.
      ...
      if ((options as any).__testHookAfterPointerAction) { await (options as any).__testHookAfterPointerAction() }
  ...
  private async _finishPointerActionDetectLayoutShift(progress: Progress, actionName: ActionName, point: types.Point, options: types.PointerActionOptions & types.PointerActionWaitOptions & types.NavigatingActionWaitOptions, action: (point: types.Point) => Promise<void>): Promise<'error:notconnected' | { hitTargetDescription: string } | 'done'> {
    ...
    if (!options.force) {
      ...
      if (actionName === 'move and up') {
        ...
        if (!handle._objectId) { return handle.rawValue() as 'error:notconnected' }
  ...
  async _selectOption(progress: Progress, elements: ElementHandle[], values: types.SelectOption[], options: types.NavigatingActionWaitOptions & types.ForceOptions): Promise<string[] | 'error:notconnected'> {
    ...
    return this._page._frameManager.waitForSignalsCreatedBy(progress, options.noWaitAfter, async () => {
      ...
      const result = await this.evaluatePoll(progress, ([injected, node, { optionsToSelect, force }]) => injected.waitForElementStatesAndPerformAction(node, ['visible', 'enabled'], force, injected.selectOptions.bind(injected, optionsToSelect)), { optionsToSelect, force: options.force })
      await this._page._doSlowMo()
      return result
    })
  }
  ...
  async _fill(progress: Progress, value: string, options: types.NavigatingActionWaitOptions & types.ForceOptions): Promise<'error:notconnected' | 'done'> {
    ...
    return this._page._frameManager.waitForSignalsCreatedBy(progress, options.noWaitAfter, async () => {
      ...
      const filled = await this.evaluatePoll(progress, ([injected, node, { value, force }]) => injected.waitForElementStatesAndPerformAction(node, ['visible', 'enabled', 'editable'], force, injected.fill.bind(injected, value)), { value, force: options.force })
      progress.throwIfAborted() // Avoid action that has side-effects.
  ...
  async selectText(metadata: CallMetadata, options: types.TimeoutOptions & types.ForceOptions = {}): Promise<void> {
    ...
    return controller.run(async (progress) => {
      progress.throwIfAborted() // Avoid action that has side-effects.
      const result = await this.evaluatePoll(progress, ([injected, node, force]) => injected.waitForElementStatesAndPerformAction(node, ['visible'], force, injected.selectText.bind(injected)), options.force)
      assertDone(throwRetargetableDOMError(result))
    }, this._page._timeoutSettings.timeout(options))
  }
  ...
  async _setInputFiles(progress: Progress, files: SetInputFilesFiles, options: types.NavigatingActionWaitOptions): Promise<'error:notconnected' | 'done'> {
    ...
    const result = await this.evaluateHandleInUtility(([injected, node, multiple]): Element | undefined => {
      ...
      if (multiple && !(element as HTMLInputElement).multiple) { throw injected.createStacklessError('Non-multiple file input can only accept single file') }
      return element
    }, files.length > 1)
  ...
  async waitForElementState(metadata: CallMetadata, state: 'visible' | 'hidden' | 'stable' | 'enabled' | 'disabled' | 'editable', options: types.TimeoutOptions = {}): Promise<void> {
    ...
    return controller.run(async (progress) => {
      ...
      const result = await this.evaluatePoll(progress, ([injected, node, state]) => injected.waitForElementStatesAndPerformAction(node, [state], false, () => 'done' as const), state)
      assertDone(throwRetargetableDOMError(result))
    }, this._page._timeoutSettings.timeout(options))
  }
  ...
  async _waitForDisplayedAtStablePosition(progress: Progress, force: boolean, waitForEnabled: boolean): Promise<'error:notconnected' | 'done'> {
    ...
    const result = await this.evaluatePoll(progress, ([injected, node, { waitForEnabled, force }]) => injected.waitForElementStatesAndPerformAction(
      node,
      waitForEnabled ? ['visible', 'stable', 'enabled'] : ['visible', 'stable'],
      force,
      () => 'done' as const,
...
export function waitForSelectorTask(selector: SelectorInfo, state: 'attached' | 'detached' | 'visible' | 'hidden', omitReturnValue?: boolean, root?: ElementHandle): SchedulableTask<Element | undefined> {
  return (injectedScript) => injectedScript.evaluateHandle((injected, {
    parsed, strict, state, omitReturnValue, root,
  }) => {
