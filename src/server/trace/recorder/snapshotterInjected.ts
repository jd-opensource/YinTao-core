...
export function frameSnapshotStreamer(snapshotStreamer: string) {
  ...
  if ((window as any)[snapshotStreamer]) { return }
    ...
    captureSnapshot(): SnapshotData | undefined {
      ...
      const visitNode = (node: Node | ShadowRoot): { equals: boolean, n: NodeSnapshot } | undefined => {
        ...
        if (nodeType !== Node.ELEMENT_NODE
            ...
            && nodeType !== Node.TEXT_NODE) { return }
        ...
        if (nodeName === 'META' && (node as HTMLMetaElement).httpEquiv.toLowerCase() === 'content-security-policy') { return }
        ...
        if (nodeName === 'TEXTAREA') {
          const { value } = node as HTMLTextAreaElement
          expectValue(value)
          ...
          let documentOrShadowRoot :any = null
          ...
          if (documentOrShadowRoot) {
            for (const sheet of (documentOrShadowRoot as any).adoptedStyleSheets || []) { visitChildStyleSheet(sheet) }
            expectValue(kEndOfList)
          }
        }
        ...
        if (nodeType === Node.ELEMENT_NODE) {
          ...
          for (let i = 0; i < element.attributes.length; i++) {
            ...
            let { value } = element.attributes[i]
            if (nodeName === 'META') { value = this.__sanitizeMetaAttribute(name, value, (node as HTMLMetaElement).httpEquiv) } else if (name === 'src' && (nodeName === 'IMG')) { value = this._sanitizeUrl(value) } else if (name === 'srcset' && (nodeName === 'IMG')) { value = this._sanitizeSrcSet(value) } else if (name === 'srcset' && (nodeName === 'SOURCE')) { value = this._sanitizeSrcSet(value) } else if (name === 'href' && (nodeName === 'LINK')) { value = this._sanitizeUrl(value) } else if (name.startsWith('on')) { value = '' }
            expectValue(name)
            expectValue(value)
            attrs[name] = value
          }
      ...
      const visitStyleSheet = (sheet: CSSStyleSheet) => {
        ...
        if (cssText === oldCSSText) { return { equals: true, n: [[snapshotNumber - data.ref![0], data.ref![1]]] } }
        data.ref = [snapshotNumber, nodeCounter++]
        return {
          equals: false,
          n: ['template', {
