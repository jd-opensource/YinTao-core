...
export class CRExecutionContext implements js.ExecutionContextDelegate {
  ...
  async rawEvaluateJSON(expression: string): Promise<any> {
    ...
    if (exceptionDetails) { throw new js.JavaScriptErrorInEvaluate(getExceptionMessage(exceptionDetails)) }
...
function rewriteError(error: Error): Protocol.Runtime.evaluateReturnValue {
  ...
  if (error instanceof TypeError && error.message.startsWith('Converting circular structure to JSON')) { rewriteErrorMessage(error, `${error.message} Are you passing a nested JSHandle?`) }
  if (!js.isJavaScriptErrorInEvaluate(error) && !isSessionClosedError(error)) { throw new Error('Execution context was destroyed, most likely because of a navigation.') }
  throw error
}
