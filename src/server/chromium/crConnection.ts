...
export class CRSession extends EventEmitter {
  ...
  async send<T extends keyof Protocol.CommandParameters>(
    ...
    if (this._browserDisconnectedLogs !== undefined) { throw new ProtocolError(true, `Browser closed.${this._browserDisconnectedLogs}`) }
    if (!this._connection) { throw new ProtocolError(true, 'Target closed') }
    const id = this._connection._rawSend(this._sessionId, method, params)
    return new Promise((resolve, reject) => {
      this._callbacks.set(id, {
        resolve, reject, error: new ProtocolError(false), method,
      })
    })
  }
