...
import {
  Page, PageBinding, PageDelegate, Worker,
} from '../page'
...
export class CRPage implements PageDelegate {
  ...
  constructor(client: CRSession, targetId: string, browserContext: CRBrowserContext, opener: CRPage | null, hasUIWindow: boolean, isBackgroundPage: boolean) {
    ...
    if (opener && !browserContext._options.noDefaultViewport) {
      ...
      if (viewportSize) { this._page._state.emulatedSize = { viewport: viewportSize, screen: viewportSize } }
    }
  ...
  private _reportAsNew(error?: Error) {
    if (this._isBackgroundPage) {
      if (!error) { this._browserContext.emit(CRBrowserContext.CREvents.BackgroundPage, this._page) }
    } else {
  ...
  async getFrameElement(frame: frames.Frame): Promise<dom.ElementHandle> {
    ...
    const { backendNodeId } = await parentSession._client.send('DOM.getFrameOwner', { frameId: frame._id }).catch((e) => {
      if (e instanceof Error && e.message.includes('Frame with the given id was not found.')) { rewriteErrorMessage(e, 'Frame has been detached.') }
      throw e
    })
...
class FrameSession {
  ...
  async _initialize(hasUIWindow: boolean) {
    ...
    if (this._isMainFrame()) { promises.push(this._client.send('Emulation.setFocusEmulationEnabled', { enabled: true })) }
    const options = this._crPage._browserContext._options
    if (options.bypassCSP) { promises.push(this._client.send('Page.setBypassCSP', { enabled: true })) }
    if (options.ignoreHTTPSErrors) { promises.push(this._client.send('Security.setIgnoreCertificateErrors', { ignore: true })) }
    if (this._isMainFrame()) { promises.push(this._updateViewport()) }
    if (options.hasTouch) { promises.push(this._client.send('Emulation.setTouchEmulationEnabled', { enabled: true })) }
    if (options.javaScriptEnabled === false) { promises.push(this._client.send('Emulation.setScriptExecutionDisabled', { value: true })) }
    if (options.userAgent || options.locale) { promises.push(this._client.send('Emulation.setUserAgentOverride', { userAgent: options.userAgent || '', acceptLanguage: options.locale })) }
  ...
  _onLogEntryAdded(event: Protocol.Log.entryAddedPayload) {
    const {
      level, text, args, source, url, lineNumber,
    } = event.entry
    if (args) { args.map((arg) => releaseObject(this._client, arg.objectId!)) }
    if (source !== 'worker') {
  ...
  async _updateExtraHTTPHeaders(initial: boolean): Promise<void> {
    ...
    if (!initial || headers.length) { await this._client.send('Network.setExtraHTTPHeaders', { headers: headersArrayToObject(headers, false /* lowerCase */) }) }
  }
  ...
  async _updateViewport(): Promise<void> {
    ...
    if (this._windowId) {
      ...
      if (this._crPage._browserContext._browser.options.headful) {
        ...
        if (process.platform === 'win32') { insets = { width: 16, height: 88 } } else if (process.platform === 'linux') { insets = { width: 8, height: 85 } } else if (process.platform === 'darwin') { insets = { width: 2, height: 80 } }
  ...
  async _getBoundingBox(handle: dom.ElementHandle): Promise<types.Rect | null> {
    ...
    return {
      x: x + position.x, y: y + position.y, width, height,
    }
  }
