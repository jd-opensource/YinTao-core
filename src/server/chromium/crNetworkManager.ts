...
export class CRNetworkManager {
  ...
  _onRequest(workerFrame: frames.Frame | undefined, requestWillBeSentEvent: Protocol.Network.requestWillBeSentPayload, requestPausedEvent: Protocol.Fetch.requestPausedPayload | null) {
    ...
    if (!frame && requestPausedEvent && requestPausedEvent.frameId) { frame = this._page._frameManager.frame(requestPausedEvent.frameId) }
    ...
    if (requestPausedEvent && requestPausedEvent.request.method === 'OPTIONS' && this._page._needsRequestInterception()) {
      ...
      if (requestHeaders['Access-Control-Request-Headers']) { responseHeaders.push({ name: 'Access-Control-Allow-Headers', value: requestHeaders['Access-Control-Request-Headers'] }) }
    ...
    if (!frame) {
      if (requestPausedEvent) { this._client._sendMayFail('Fetch.continueRequest', { requestId: requestPausedEvent.requestId }) }
      return
    }
    ...
    if (requestPausedEvent) {
      ...
      if (redirectedFrom || (!this._userRequestInterceptionEnabled && this._protocolRequestInterceptionEnabled)) { this._client._sendMayFail('Fetch.continueRequest', { requestId: requestPausedEvent.requestId }) } else { route = new RouteImpl(this._client, requestPausedEvent.requestId) }
    }
  ...
  _onLoadingFailed(workerFrame: frames.Frame | undefined, event: Protocol.Network.loadingFailedPayload) {
    ...
    if (!request) { return }
    const response = request.request._existingResponse()
    if (response) { response._requestFinished(helper.secondsToRoundishMillis(event.timestamp - request._timestamp)) }
    this._requestIdToRequest.delete(request._requestId)
    if (request._interceptionId) { this._attemptedAuthentications.delete(request._interceptionId) }
...
class InterceptableRequest {
  ...
  constructor(options: {
    ...
    const {
      frame, documentId, route, requestWillBeSentEvent, requestPausedEvent, redirectedFrom,
    } = options
