...
import {
  Browser, BrowserOptions, BrowserProcess, PlaywrightOptions,
} from '../browser'
import * as types from '../types'
import {
  debugMode, fetchData, getUserAgent, headersArrayToObject, HTTPRequestParams, removeFolders, streamToString,
} from '../../utils/utils'
...
export class Chromium extends BrowserType {
  ...
  override async connectOverCDP(metadata: CallMetadata, endpointURL: string, options: { slowMo?: number, headers?: types.HeadersArray }, timeout?: number) {
    ...
    return controller.run(async (progress) => await this._connectOverCDPInternal(progress, endpointURL, options), TimeoutSettings.timeout({ timeout }))
  }
  ...
  async _connectOverCDPInternal(progress: Progress, endpointURL: string, options: { slowMo?: number, headers?: types.HeadersArray }, onClose?: () => Promise<void>) {
    ...
    if (options.headers) { headersMap = headersArrayToObject(options.headers, false) }

    if (!headersMap) { headersMap = { 'User-Agent': getUserAgent() } } else if (headersMap && !Object.keys(headersMap).some((key) => key.toLowerCase() === 'user-agent')) { headersMap['User-Agent'] = getUserAgent() }
  ...
  private _createDevTools() {
    ...
    const { directory } = registry.findExecutable('chromium')
      ...
      if (capabilities['se:cdp']) {
        ...
        if (endpointURL.hostname === 'localhost' || endpointURL.hostname === '127.0.0.1') {
          ...
          try {
            ...
            const { proxyId } = JSON.parse(sessionResponse)
  ...
  private _innerDefaultArgs(options: types.LaunchOptions): string[] {
    ...
    if (os.platform() === 'darwin') { chromeArguments.push('--enable-use-zoom-for-dsf=false') }
