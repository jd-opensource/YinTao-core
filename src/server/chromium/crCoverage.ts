...
class JSCoverage {
  ...
  async stop(): Promise<types.JSCoverageEntry[]> {
    ...
    for (const entry of profileResponse.result) {
      ...
      if (source) { coverage.push({ ...entry, source }) } else { coverage.push(entry) }
    }
    return coverage
  }
}
...
function convertToDisjointRanges(nestedRanges: {
  ...
  for (const point of points) {
    if (hitCountStack.length && lastOffset < point.offset && hitCountStack[hitCountStack.length - 1] > 0) {
      ...
      if (lastResult && lastResult.end === lastOffset) { lastResult.end = point.offset } else { results.push({ start: lastOffset, end: point.offset }) }
    }
