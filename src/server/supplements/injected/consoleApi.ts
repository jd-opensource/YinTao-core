  ...
  class Locator {
    ...
    constructor(selector: string, options?: { hasText?: string | RegExp, has?: Locator }) {
      ...
      if (options?.hasText) {
        ...
        if (text instanceof RegExp) { this.selector += ` >> :scope:text-matches(${escapeWithQuotes(text.source, '"')}, "${text.flags}")` } else { this.selector += ` >> :scope:has-text(${escapeWithQuotes(text)})` }
      }
