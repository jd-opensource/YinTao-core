...
import { CodeGenerator, ActionInContext } from './recorder/apiCodeGenerator'
...
import {
  CallLog, CallLogStatus, EventData, Mode, Source, UIState,
...
const symbol = Symbol('ApiRecorderSupplement')

export const _a = 0
export class ApiRecorderSupplement implements InstrumentationListener {
  ...
  static showInspector(context: BrowserContext) {
    ApiRecorderSupplement.show(context, {}).catch(() => {})
  }

  static show(context: BrowserContext, params: channels.BrowserContextRecorderSupplementEnableParams = {}): Promise<ApiRecorderSupplement> {
    let recorderPromise = (context as any)[symbol] as Promise<ApiRecorderSupplement>
    if (!recorderPromise) {
      const recorder = new ApiRecorderSupplement(context, params)
  ...
  private _updateUserSources() {
    ...
    for (const metadata of this._currentCallsMetadata.keys()) {
      ...
      if (!source) {
        source = {
          file, text: this._readSource(file), highlight: [], language: languageForFile(file),
        }
...
class ContextRecorder extends EventEmitter {
  ...
  private async _describeFrame(frame: Frame): Promise<actions.FrameDescription> {
    ...
    const chain: Frame[] = []
    for (let ancestor: Frame | null = frame; ancestor; ancestor = ancestor.parentFrame()) { chain.push(ancestor) }
    chain.reverse()
  ...
  private async _performAction(frame: Frame, action: actions.Action) {
    ...
    if (action.name === 'check') { await perform('check', { selector: action.selector }, (callMetadata) => frame.check(callMetadata, action.selector, { timeout: kActionTimeout, strict: true })) }
    if (action.name === 'uncheck') { await perform('uncheck', { selector: action.selector }, (callMetadata) => frame.uncheck(callMetadata, action.selector, { timeout: kActionTimeout, strict: true })) }
