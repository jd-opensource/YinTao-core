...
import {
  allEngineNames, InvalidSelectorError, ParsedSelector, parseSelector, stringifySelector,
...
export class Selectors {
  ...
  async query(frame: frames.Frame, info: SelectorInfo, scope?: dom.ElementHandle): Promise<dom.ElementHandle<Element> | null> {
    ...
    const handle = await injectedScript.evaluateHandle((injected, { parsed, scope, strict }) => injected.querySelector(parsed, scope || document, strict), { parsed: info.parsed, scope, strict: info.strict })
  ...
  async _queryArrayInMainWorld(frame: frames.Frame, info: SelectorInfo, scope?: dom.ElementHandle): Promise<js.JSHandle<Element[]>> {
    ...
    const injectedScript = await context.injectedScript()
    const arrayHandle = await injectedScript.evaluateHandle((injected, { parsed, scope }) => injected.querySelectorAll(parsed, scope || document), { parsed: info.parsed, scope })
    return arrayHandle
  }
  ...
  async _queryCount(frame: frames.Frame, info: SelectorInfo, scope?: dom.ElementHandle): Promise<number> {
    ...
    const injectedScript = await context.injectedScript()
    return await injectedScript.evaluate((injected, { parsed, scope }) => injected.querySelectorAll(parsed, scope || document).length, { parsed: info.parsed, scope })
  }
  ...
  parseSelector(selector: string | ParsedSelector, strict: boolean): SelectorInfo {
    ...
    for (const name of allEngineNames(parsed)) {
      ...
      if (!custom && !this._builtinEngines.has(name)) { throw new InvalidSelectorError(`Unknown engine "${name}" while parsing selector ${stringifySelector(parsed)}`) }
