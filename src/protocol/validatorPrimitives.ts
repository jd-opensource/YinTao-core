...
export const tAny: Validator = (arg: any) => arg
export const tOptional = (v: Validator): Validator => (arg: any, path: string) => {
  if (Object.is(arg, undefined)) return arg
  return v(arg, path)
}
export const tArray = (v: Validator): Validator => (arg: any, path: string) => {
  if (!Array.isArray(arg)) throw new ValidationError(`${path}: expected array, got ${typeof arg}`)
  return arg.map((x, index) => v(x, `${path}[${index}]`))
}
export const tObject = (s: { [key: string]: Validator }): Validator => (arg: any, path: string) => {
...
export const tEnum = (e: string[]): Validator => (arg: any, path: string) => {
