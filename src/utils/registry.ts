...
import {
  getFromENV, getAsBooleanFromENV, calculateSha1, removeFolders, existsAsync, hostPlatform, canAccessFile, spawnAsync, fetchData, wrapInASCIIBox, transformCommandsForRoot,
} from './utils'
import {
  DependencyGroup, installDependenciesLinux, installDependenciesWindows, validateDependenciesLinux, validateDependenciesWindows,
} from './dependencies'
...
export class Registry {
  ...
  constructor(browsersJSON: BrowsersJSON) {
    ...
    const executablePathOrDie = (name: string, e: string | undefined, installByDefault: boolean, sdkLanguage: string) => {
      if (!e) { throw new Error(`${name} is not supported on ${hostPlatform}`) }
  ...
  private _createChromiumChannel(name: ChromiumChannel, lookAt: Record<'linux' | 'darwin' | 'win32', string>, install?: () => Promise<void>): ExecutableImpl {
    const executablePath = (sdkLanguage: string, shouldThrow: boolean) => {
      ...
      for (const prefix of prefixes) {
        ...
        if (canAccessFile(executablePath)) { return executablePath }
      }
  ...
  private async _validateHostRequirements(sdkLanguage: string, browserName: BrowserName, browserDirectory: string, linuxLddDirectories: string[], dlOpenLibraries: string[], windowsExeAndDllDirectories: string[]) {
    ...
    if (os.platform() === 'linux') { return await validateDependenciesLinux(sdkLanguage, linuxLddDirectories.map((d) => path.join(browserDirectory, d)), dlOpenLibraries) }
    if (os.platform() === 'win32' && os.arch() === 'x64') { return await validateDependenciesWindows(windowsExeAndDllDirectories.map((d) => path.join(browserDirectory, d))) }
  }
  ...
  async installDeps(executablesToInstallDeps: Executable[], dryRun: boolean) {
    ...
    targets.add('tools')
    if (os.platform() === 'win32') { return await installDependenciesWindows(targets, dryRun) }
  ...
  private async _downloadExecutable(descriptor: BrowsersJSONDescriptor, executablePath: string | undefined, downloadPathTemplate: string | undefined, downloadHostEnv: string) {
    if (!downloadPathTemplate || !executablePath) { throw new Error(`ERROR: Playwright does not support ${descriptor.name} on ${hostPlatform}`) }
    const downloadHost = (downloadHostEnv && getFromENV(downloadHostEnv))
    ...
    for (const fileName of await fs.promises.readdir(linksDir)) {
      ...
      try {
        ...
        for (const browserName of allDownloadable) {
          ...
          if (!shouldHaveMarkerFile || (await existsAsync(markerFilePath(usedBrowserPath)))) { usedBrowserPaths.add(usedBrowserPath) }
        }
      } catch (e) {
