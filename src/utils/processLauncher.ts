...
import { removeFolders } from '../utils/suger'
...
export async function gracefullyCloseAll() {
  await Promise.all(Array.from(gracefullyCloseSet).map((gracefullyClose) => gracefullyClose().catch(() => {})))
}
...
export async function launchProcess(options: LaunchProcessOptions): Promise<LaunchResult> {
  ...
  if (!spawnedProcess.pid) {
    ...
    const failedPromise = new Promise<Error>((f) => failed = f)
  ...
  if (options.handleSIGTERM) { listeners.push(eventsHelper.addEventListener(process, 'SIGTERM', gracefullyClose)) }
  if (options.handleSIGHUP) { listeners.push(eventsHelper.addEventListener(process, 'SIGHUP', gracefullyClose)) }
