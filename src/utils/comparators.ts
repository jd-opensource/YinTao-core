...
import {
  diff_match_patch, DIFF_INSERT, DIFF_DELETE, DIFF_EQUAL,
...
function compareBuffersOrStrings(actualBuffer: Buffer | string, expectedBuffer: Buffer): ComparatorResult {
  ...
  if (!actualBuffer || !(actualBuffer instanceof Buffer)) { return { errorMessage: 'Actual result should be a Buffer or a string.' } }
...
function compareImages(mimeType: string, actualBuffer: Buffer | string, expectedBuffer: Buffer, options: ImageComparatorOptions = {}): ComparatorResult {
  if (!actualBuffer || !(actualBuffer instanceof Buffer)) { return { errorMessage: 'Actual result should be a Buffer.' } }
  ...
  let maxDiffPixels
  if (maxDiffPixels1 !== undefined && maxDiffPixels2 !== undefined) { maxDiffPixels = Math.min(maxDiffPixels1, maxDiffPixels2) } else { maxDiffPixels = maxDiffPixels1 ?? maxDiffPixels2 ?? 0 }
